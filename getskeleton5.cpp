...
bool Nui_GotDepthAlert(unsigned short  Depthes[][imageHeight]);
bool Nui_GotVideoAlert(unsigned char  RGBinfo[][imageHeight][3]);
bool Nui_GotSkeletonAlert(double Skeldat[Skely][Skelx]);
...
bool Nui_GotVideoAlert(unsigned char RGBinfo[][imageHeight][3])
{
    ...
    if( LockedRect.Pitch != 0 )
    {
		...
		for (int j= 0;j< imageHeight*4;j+= 4){
			...
			{
				RGBinfo[a][b][0]= pBuffer[base+i+2]; //R
				RGBinfo[a][b][1]= pBuffer[base+i+1]; //G
				RGBinfo[a][b][2]= pBuffer[base+i+0]; //B
				b++;
			}
			a++;
		}
	...
	else
	{
        ...
        for( int i = 0 ; i < NUI_SKELETON_COUNT ; i++ )
        {
            if( SkeletonFrame.SkeletonData[i].eTrackingState == NUI_SKELETON_TRACKED )
            {
				...
				long colorX, colorY;
				NuiImageGetColorPixelCoordinatesFromDepthPixel(NUI_IMAGE_RESOLUTION_640x480, NULL, (long)fx, (long)fy, 
						pSkel->SkeletonPositions[j].z, &colorX, &colorY);

                    Skeldat[5][j+r] = (double)(colorX); 
                    Skeldat[6][j+r] = (double)(colorY); 
...
{	
		...
		double  *Rgb, *Option, *Skel;
		unsigned short *Dep;
		int Depdims[2]   = {imageHeight, imageWidth};
		int RGBdims[3]   = {imageHeight, imageWidth, 3};
		int Sdims[2]     = {Skelx, Skely};
		plhs[0] = mxCreateNumericArray(2, Depdims, mxUINT16_CLASS, mxREAL);
		plhs[1] = mxCreateNumericArray(3, RGBdims, mxUINT8_CLASS, mxREAL);
		plhs[2] = mxCreateNumericArray(2, Sdims, mxDOUBLE_CLASS, mxREAL);
		Dep  = (unsigned short *) mxGetPr(plhs[0]);
		Rgb  = mxGetPr(plhs[1]);
		Skel = mxGetPr(plhs[2]);
		Option = mxGetPr(prhs[0]);
		unsigned short (*Depxy)[imageHeight] = (unsigned short (*)[imageHeight])Dep;
		unsigned char (*Rgbxyz)[imageHeight][3] = (unsigned char (*)[imageHeight][3])Rgb;
		double (*Skelxy)[Skelx] = (double (*)[Skelx])Skel;
